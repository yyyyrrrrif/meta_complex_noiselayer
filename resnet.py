...
class NoiseLayer(MetaModule):
    def __init__(self, bias_init, *args, **kwargs):
        super().__init__()
        ignore = nn.Linear(*args, **kwargs)
        w_out, w_in = ignore.weight.data.size()
        b_l = ignore.bias.data.size()[0]
        # 明天看s-model里是怎么取的
        weight_init = torch.zeros(w_out, w_in) + 1e-8

        self.register_buffer('weight', to_var(weight_init, requires_grad=True))
        self.register_buffer('bias', to_var(bias_init, requires_grad=True))
...
class ResNet(MetaModule):
    ...
    def forward(self, x):
        ...
        ftrs = out.view(out.size(0), -1)
        out = self.linear(ftrs)
